...
export const DataConfigPanel: React.FC = () => {
  ...
  if (!config) {
    return (
      <div className="p-4 rounded-2xl bg-[#0F111A] border border-white/5 text-xs text-gray-500">
        Loading data config…
      </div>
    );
  }
    ...
    <div className="p-4 rounded-2xl bg-[#0F111A] border border-white/5 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          disabled={saving}
          className="flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-lg bg-white/5 hover:bg-white/10 text-gray-300 disabled:opacity-40"
        >
      ...
      <div className="space-y-2">
        <div className="text-[11px] font-semibold text-gray-300">Income band cutoffs (₹/month pocket money)</div>
        <div className="flex items-center gap-2 flex-wrap">
          {config.incomeBandLabels.map((label, i) => (
            <React.Fragment key={label}>
              <span className="text-[10px] px-2 py-1 rounded-md bg-black/40 text-gray-300 border border-white/5">
                {label}
              </span>
              {i < config.incomeBandCutoffs.length && (
                <input
                  ...
                  className="w-20 px-2 py-1 rounded-md bg-white/5 border border-white/10 text-[11px] text-white text-center"
                />
              )}
            </React.Fragment>
          ))}
        ...
        <button
          ...
          disabled={saving}
          className="text-[10px] font-bold px-2.5 py-1 rounded-lg bg-primary-600 hover:bg-primary-500 text-white disabled:opacity-40"
        >
          Save cutoffs
        </button>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <span className="text-[11px] font-semibold text-gray-300">Outlier handling (Tukey fence)</span>
          <button
            onClick={() =>
        ...
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-gray-500">Exclude beyond</span>
          <input
            type="number"
            step="0.1"
            ...
            className="w-16 px-2 py-1 rounded-md bg-white/5 border border-white/10 text-[11px] text-white text-center"
          />
          <span className="text-[10px] text-gray-500">× IQR from Q1/Q3</span>
          <button
            onClick={() => save({ outlierHandling: config.outlierHandling })}
            disabled={saving}
            className="ml-auto text-[10px] font-bold px-2.5 py-1 rounded-lg bg-primary-600 hover:bg-primary-500 text-white disabled:opacity-40"
          >
            Save
          </button>
        </div>
          ...
          {STARTER_CATEGORIES.filter((c) => c.id !== "miscellaneous").map((cat) => {
            ...
            return (
              <div key={cat.id} className="flex items-center gap-2">
                <span className="text-[10px] text-gray-400 w-28 truncate">{cat.short}</span>
                <input
                  type="range"
                  min={0}
                  max={1}
                ...
                />
                <span className="text-[10px] text-gray-500 w-8 text-right">{weight.toFixed(2)}</span>
              </div>
            );
          })}
        </div>
