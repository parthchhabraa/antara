...
Step 16 — ported from the unmerged survey branch (origin/claude/
antara-spending-survey-6o4o2w) into this repo's main. The "replace the
guessed CATEGORIES_METADATA numbers" use case this docstring originally
described is now superseded — that's what backend/app/ml/survey_etl.py +
POST /api/v1/admin/recompute-benchmarks do automatically at runtime (Step
10, refreshed to n=36 in Step 15), no manual median-copying required
anymore. What this script is still for: a raw, row-per-respondent CSV/JSONL
dump for anything survey_etl.py's aggregate-only output can't answer —
ad-hoc notebook analysis, sharing an anonymized cut with a collaborator, or
prepping real per-respondent data for the "Stage 2 trained embeddings" work
survey_etl.py's own module docstring says is still out of scope until live
transaction volume supports it. On sample size: below ~100 responses,
treat any derived stats from this export as directional, not authoritative
— current live count is 36 as of Step 15/16 (check `/api/v1/admin/status`
or `admin/categoryBenchmarks` for the current number).
"""
